...
# Load structured course metadata from JSON file
with open('semantic_search/models/subhash_course_metadata.json', 'r') as f:
    courses = json.load(f)

# Load pre-trained BERT model and tokenizer for querying
...
def getCourses(query: str, topK: int):
    try:
        ...
        # Retrieve matched courses and format the response
        results = []
        for idx, dist in zip(indices[0], distances[0]):
            course = courses[idx]  # Get course metadata (now structured)
            
            # Format the course dictionary
            course_result = {
...
# Print the matched courses in the specified format
