...
embeddings_dictionary = pickle.load(open('semantic_search/models/models/embeddings_dictionary.pkl', 'rb'))
...
def getCourses(query: str, topK: int):
	...
	query_embedding = get_glove_embedding(query_processed, embeddings_dictionary).reshape(1, -1)
	courses['similarity'] = cosine_similarity(courses['embedding'].tolist(), query_embedding).flatten()
